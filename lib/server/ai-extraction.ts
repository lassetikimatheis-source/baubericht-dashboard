      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            ...
            "Erlaubte cluster: Schadstoffsanierung / Asbest, Asbestarbeiten, Bodenbelagsarbeiten, Malerarbeiten, Fliesen und Estricharbeiten, Heizung und Sanitär, Elektroarbeiten, Tischlerarbeiten, Fassadenarbeiten, Dacharbeiten, Fensterarbeiten, Rückbau / Entsorgung, Außenanlagen, Reinigung, Planung / Dokumentation, Sonstige.",
...
function detectMeasures(text: string): DetectedMeasure[] {
  const mappings: Array<{ pattern: RegExp; cluster: MeasureCluster; description: string; projectType: string }> = [
    { pattern: /(?:Summe\s*2\.?\s*)?Asbest(?:\s*sanierung|sanierung|arbeiten)?|Schadstoff(?:sanierung|entsorgung|arbeiten)?|Gefahrstoff(?:arbeiten)?|TRGS\s*519|\bPCB\b|\bKMF\b|\bBT\s*(?:11|17\.45)\b|Flexplatten|asbesthaltig|Beprobung\s+auf\s+Asbest/i, cluster: "Schadstoffsanierung / Asbest", description: "Summe 2. Asbestsanierung", projectType: "Schadstoffsanierung / Asbest" },
...
function parseOfferMeasuresLegacy(text: string): AiMeasureResult[] {
  const mappings: Array<{ section: number; heading: RegExp; cluster: MeasureCluster; description: string }> = [
    ...
    { section: 2, heading: /Summe\s+2\.?\s+Asbest\s*sanierung[^\n\r]*?([\d.]+,\d{2})\s*€/i, cluster: "Schadstoffsanierung / Asbest", description: "Summe 2. Asbestsanierung" },
    { section: 2, heading: /Summe\s+2\.?\s+Asbestsanierung[^\n\r]*?([\d.]+,\d{2})\s*€/i, cluster: "Schadstoffsanierung / Asbest", description: "Summe 2. Asbestsanierung" },
